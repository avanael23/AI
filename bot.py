...
# Change the final part of bot.py:
if __name__ == '__main__':
    # ... other setup ...

    # New Webhook Setup
    # Read the port provided by Render (usually PORT environment variable)
    PORT = int(os.environ.get('PORT', '8080')) 

    # Start the web server on the port Render requires
    print(f"Starting bot using webhooks on port {PORT}...")
    app.run_webhook(
        listen="0.0.0.0",
        port=PORT,
        url_path=TOKEN,
        webhook_url=f"YOUR_RENDER_URL/{{TOKEN}}" # Replace with your Render URL
    )
